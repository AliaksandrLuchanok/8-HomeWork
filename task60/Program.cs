...
/* Создание трехмерной матрицы с заполненнием случайных исключительных чисел
в диапазоне от 0 до 20. Диапазон генерации которых можно изменить в методе */
int [,,] CreatingThreeMatrix (int valueRow, int valueColl, int valueList)
{
  ...
  int lastValueRandom = 20;   // диапазон можно вынести на выбор пользователю
      ...
      for (int k = 0; k < valueList;)
      {
        ...
        for (int l = 0; l < i+1;)
        {
          for (int m = 0; m < valueColl;)
          {
            ...
            {
              // System.Console.WriteLine($"newThreeMatrix {l} {m} {n} = {newThreeMatrix [l,m,n]} сравниваем с tempRandom {tempRandom}");
        ...
        if (!checkingMatch)
        {
          ...
          // ShowThreeMatrix (newThreeMatrix);
          k++;
        }
      } 
    }  
...
// Демонстрация в консоль полученной пользовательской матрицы
